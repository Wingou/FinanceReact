...
async function connectAndCall (req, res, data) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      if (path_ === '/price') {
        // http://localhost:3002/price?id=15333
        sql = selectPriceById
        params = [query_.id]
      } else if (path_ === '/pricesbyperiod') {
        // http://localhost:3002/pricesbyperiod?beginDate=20250115&endDate=20250215
        ...
        // http://localhost:3002/pricesByMonthYear?year=2025&month=01
